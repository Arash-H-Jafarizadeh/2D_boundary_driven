...
def kraus_operator(indx, Ls):
    ...
    if indx == 2:#
        return [('N',0)]#
    if indx == 3:#
        return [('C',Ls-1),('Cd',Ls-1)]# 
    if indx == 4:#
        return [('Cd',0),('C',Ls-1),('Cd',Ls-1)]#
    if indx == 5:#
        return [('N',0),('C',Ls-1),('Cd',Ls-1)]#
    if indx == 6:#
        return [('C',Ls-1)]#
    if indx == 7:#
        return [('Cd',0),('C',Ls-1)]#
    if indx == 8:#
        return [('N',0),('C',Ls-1)]#[('Cd',0),('C',0),('C',Ls-1)]#
...
def currents(input_mps, dimensions, magnetic_field = 0.0):
    ...
    for indx, (row, x, y) in enumerate(indexes): 
        var_l = input_mps.expectation_value_term([('Cd',x),('C',y)], autoJW=True)
        var_r = input_mps.expectation_value_term([('Cd',y),('C',x)], autoJW=True)
...
def correlations(input_mps, dimensions):
    ...
    for x in range(L):    
        for y in range(L):
            exp_val = input_mps.expectation_value_term([('Cd',x),('C',y)], autoJW=True)
            C_Mat[x,y] += exp_val
...
def mpo_simulation(time_steps, system_dimensions, physical_couplings, **Kwargs):
    ...
    trunc_err = Kwargs.get('max_trunc_err', 0.789)
