...
time_steps = T_input
t_step_str, n_traj_str = f'{str(time_steps)[0]}e{int(np.log10(time_steps))}', '2e1'#'1e2'
...
if __name__ == '__main__': ############################################ Read and save data from array runs - new algorithm for fixed step_size/L/V - 20250407
    ...
    for indx, nome in enumerate(all_files):
        ...
        # N_avg[indx] = A_data['density'][0]
        # print(np.shape(A_data['density'][0]))
        # N_sq_avg[indx] = A_data['density'][1]        
        # print(np.shape(A_data['density'][1]))
        # J_avg[indx] = A_data['current'][0]
        # print(np.shape(A_data['current'][0]))
        # J_sq_avg[indx] = A_data['current'][1]
        # print(np.shape(A_data['current'][1]))
    ...
    if save_data: ###################################### SAVING ###############################################
        ...
        arcivo = open(save_path + f'Data_L{L_input:01}_V{V_input:.1f}_B{B_input:.3f}_P{P_input:.2f}_T{steps_str}_X{X_input:03}_N{trajs_str}_{K_type}_dict.npy', 'wb')  
