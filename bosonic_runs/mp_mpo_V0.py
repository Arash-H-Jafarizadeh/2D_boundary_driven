...
print(f" ***** array job {array_number} for size={input_L}, p/dt={input_p}, V={input_V}, B={input_B}, T={time_steps} and X={input_X} ***** ")
print("")
...
def single_trajectory(seed):
    ...
    loop_N, loop_J, loop_k = mbos.mpo_simulation( time_steps, (Lx, Ly), (Vs, Js), 
                                                    dt=dt, 
...
if __name__ == '__main__':
    ...
    if ploting: ################################################# PLOTING ########################################################
        ...
        fig.savefig(plot_path + f"plot_L{input_L}_P{input_p:.2f}_V{input_V:.1f}_B{input_B:.3f}_X{input_X:03}_T{time_steps_exp}_N{num_traj_exp}_A{array_number:03}_J{job_number}.png", dpi=300, bbox_inches = 'tight')    
    ...
    if saving: ######################## SAVING ##################################
        ...
        arcivo = open(save_path + f'data_L{input_L[0]}x{input_L[1]}_V{input_V:.1f}_B{input_B:.1f}_P{input_p:.2f}_T{time_steps_exp}_X{input_X:03}_N{num_traj_exp}_A{array_number:03}.npy', 'wb')  
