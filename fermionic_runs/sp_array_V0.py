...
print(f" ***** array job {array_number} for p={input_p}, size=({Lx}x{Ly}), T={time_steps} and V={input_V} ***** ")
print("")
...
for s in range(num_traj):
    ...
    t_0 = tt.time()
    (loop_N, loop_J, loop_K) = mfer.mpo_simulation( time_steps, (Lx, Ly), (input_V, Js), 
    ...
    N_avg[s] = loop_N #/ num_traj
    N_sq_avg[s] = loop_N**2 #/ num_traj
    J_avg[s] = loop_J #/ num_traj
    J_sq_avg[s] = loop_J**2 #/ num_traj
...
print(" data sqrt shapes are:", np.shape(N_sq_avg), " , ", np.shape(J_sq_avg))
print("")
...
if saving: ######################## SAVING ##################################
        ...
        arcivo = open(save_path + f'data_L{input_L:01}_V{input_V:.1f}_B{input_B:.1f}_P{input_p:.2f}_T{time_steps_exp}_N{num_traj_exp}_A{array_number:03}.npy', 'wb')  
