...
B_list = [0.0*np.pi, 0.25*np.pi, 0.5*np.pi, 0.9999*np.pi]
T_list = [200]
...
print(f" ***** array job {array_number} for dt=p={input_p}, size=({Lx}x{Ly}), T={time_steps} and V={input_V} ***** ")
print("")
...
if __name__ == '__main__':
    ...
    with mp.Pool(processes=num_processes, initializer=init_pool_processes) as pool:
        ...
        # for indx, (K, CC,NN) in enumerate( pool.map(single_trajectory, range(num_traj)) ):
