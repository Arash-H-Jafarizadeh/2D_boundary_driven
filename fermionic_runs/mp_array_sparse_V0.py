...
def single_trajectory(seed):
    ...
    # loop_C, loop_NN, loop_k = sfer.sparse_simulation( time_steps, (Lx, Ly), (input_V, Js), 
...
if __name__ == '__main__':
    ...
    if ploting: ################################################# PLOTING ########################################################
        ...
        # fig.suptitle(f"plot for ({Lx}x{Ly}), p={input_p}, V={Vs}, B={input_B:.3f}, dt ={dt} (fermion)")
        ...
        fig.savefig(plot_path + f"plot_L{input_L:01}_P{input_p:.2f}_V{input_V:.1f}_B{input_B:.3f}_{U_type}_{measurment_type}_T{time_steps_exp}_N{num_traj_exp}_A{array_number:03}_J{job_number}.png", dpi=200, bbox_inches = 'tight')
    
    if saving: ######################## SAVING ##################################
        
        # arcivo = open(save_path + f'N_data_L{input_L:01}_V{input_V:.1f}_B{input_B:.2f}_P{input_p:.2f}_T{time_steps_exp}_N{num_traj_exp}_A{array_number:03}.npy', 'wb')  
        # np.save(arcivo, N_DATA)
        # arcivo.close()

        # arcivo = open(save_path + f'J_data_L{input_L:01}_V{input_V:.1f}_B{input_B:.2f}_P{input_p:.2f}_T{time_steps_exp}_N{num_traj_exp}_A{array_number:03}.npy', 'wb')  
        # np.save(arcivo, J_DATA)
        # arcivo.close()

        # arcivo = open(save_path + f'K_data_L{input_L:01}_V{input_V:.1f}_B{input_B:.2f}_P{input_p:.2f}_T{time_steps_exp}_N{num_traj_exp}_A{array_number:03}.npy', 'wb')  
