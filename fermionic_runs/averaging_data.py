...
t_step_str, n_traj_str = f'{str(time_steps)[0]}e{int(np.log10(time_steps))}', '1e2'#'1e3'
print(t_step_str)
...
if __name__ == '__main__': ############################################ Read and save data from array runs - new algorithm for fixed step_size/L/V - 20250407
    ...
    all_files =  sorted( glob.glob(f'data_L{L_input}_V{V_input:.1f}_B{B_input:.2f}_P{P_input:.2f}_D{dt:.2f}_T{t_step_str}_N{n_traj_str}_{krs_type}'+'*.npy', root_dir=input_path) )
    ...
    for indx, nome in enumerate(all_files[:3]):
        ...
        N_avg[indx] = A_data['density']        # N_sq_avg[indx] = A_data['density']       
        print("  - -",np.shape(A_data['density']))
        J_avg[indx] = A_data['current']        # J_sq_avg[indx] = A_data['current']
    ...
    if save_data: ###################################### SAVING ###############################################
        ...
        arcivo = open(save_path + f'Data_L{L_input:01}_V{V_input:.1f}_B{B_input:.2f}_P{P_input:.2f}_D{dt:.2f}_T{steps_str}_N{trajs_str}_{krs_type}_dict.npy', 'wb')    
