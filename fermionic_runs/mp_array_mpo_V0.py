...
import fermionic_mpo_function as mfer  # type: ignore
...
ploting = True
plot_path = "fermionic_runs/plots/mpo/"


saving = True
save_path = "fermionic_runs/raw_data/mpo/"
...
L_list = [(2,8),(2,10),(2,12),(2,14),(2,16),(3,6),(3,8),(3,10),(3,12),(3,14)]
p_list = [0.05]
V_list = [1.0]#, 4.0]
...
print(f" ***** array job {array_number} for size=({input_L}), pdt={input_p}, B={input_B}, T={time_steps} and X={input_X} ***** ")
print("")
...
def single_trajectory(seed):
    ...
    loop_N, loop_J, loop_k = mfer.mpo_simulation( time_steps, (Lx, Ly), (Vs, Js), 
                                                    dt=dt, 
                                                    driving_rate=p_input, 
...
if __name__ == '__main__':
    ...
    N_sq_avg = np.average(N_sq_avg, axis=0)    # N_DATA = np.concatenate(([N_avg],[N_sq_avg]), axis=0)

    J_avg = np.average(J_avg, axis=0)    # 
    J_sq_avg = np.average(J_sq_avg, axis=0)    # J_DATA = np.concatenate(([J_avg],[J_sq_avg]), axis=0)
    ...
    if ploting: ################################################# PLOTING ########################################################
        ...
        colors = distinct_colors(2*Lx*Ly - Lx - Ly) 
        for r in range(2*Lx*Ly - Lx - Ly):
            i=int(J_avg[0,r,0])
            f=int(J_avg[0,r,1])
        ...
        fig.suptitle(f"plot for ({Lx}x{Ly}), p={input_p}, V={Vs}, B={input_B:.1f}, X={input_X} dt ={dt} (fermion)")
        fig.savefig(plot_path + f"plot_L{input_L}_P{input_p:.2f}_V{input_V:.1f}_B{input_B:.3f}_T{time_steps_exp}_X{input_X}_N{num_traj_exp}_A{array_number:03}_J{job_number}.png", dpi=300, bbox_inches = 'tight')    
    ...
    if saving: ######################## SAVING ##################################
        ...
        arcivo = open(save_path + f'data_L{input_L[0]}x{input_L[1]}_V{input_V:.1f}_B{input_B:.3f}_P{input_p:.2f}_T{time_steps_exp}_X{input_X}_N{num_traj_exp}_A{array_number:03}.npy', 'wb')  
